...
def append_posts(df: pd.DataFrame, dedup: bool = True) -> tuple[int, str | None]:
    ...
    _ensure_db()
    df = df[["platform", "text", "timestamp"]].copy()
    df = df.dropna(subset=["text"])
    df["text"] = df["text"].astype(str).str.strip()
    df = df[df["text"].str.len() > 0]
    if df.empty:
        return 0, None
    rows = [(r["platform"], r["text"], str(r["timestamp"])) for _, r in df.iterrows()]
    try:
        with sqlite3.connect(_DB_PATH) as conn:
            before = conn.execute("SELECT COUNT(*) FROM posts").fetchone()[0]
            if dedup:
                conn.executemany(
                    "INSERT OR IGNORE INTO posts (platform, text, timestamp) VALUES (?, ?, ?)",
                    rows,
                )
            else:
                conn.executemany(
                    "INSERT INTO posts (platform, text, timestamp) VALUES (?, ?, ?)",
                    rows,
                )
